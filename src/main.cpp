...
#if defined(NDEBUG)
#error "BTCT cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ubtct) are considered zero fee (for relaying and mining)
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight >= 0 && nHeight <= 2) {
    ...
    } else if (nHeight <= 525600 && nHeight >= 3) {
        nSubsidy = 14 * COIN;
        ...
        nSubsidy = 1.75 * COIN;
    } else if (nHeight <= 3679200 && nHeight >= 2102401) {
        nSubsidy = .875 * COIN;
    } else if (nHeight <= 5256000 && nHeight >= 3679201) {
        nSubsidy = .4375 * COIN;
    } else {
        nSubsidy = .001 * COIN;
    }
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    if (nHeight > Params().LAST_POW_BLOCK() && nHeight <= 525600) {
        ret = 8 * COIN;
        ...
        ret = 1 * COIN;
    } else if (nHeight >= 2102401 && nHeight <= 3679200) {
        ret = .5 * COIN;
    } else if (nHeight >= 3679201 && nHeight <= 5256000) {
        ret = .25 * COIN;
    } else {
	   ret = .0005 * COIN;
    }
...
{
    RenameThread("btct-scriptch");
    scriptcheckqueue.Thread();
}

bool RecalculateBTCTSupply(int nHeightStart)
{
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // BTCT: We use certain sporks during IBD, so check to see if they are
