...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution.
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("BTCTMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        return error("BTCTMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("BTCTMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("btct-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running BTCTMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
