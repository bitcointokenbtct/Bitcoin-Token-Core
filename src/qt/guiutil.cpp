...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a BTCT address (e.g. %1)").arg("BbXF2R7qQnB7ZJjs6G2Gispvm54n8wad9E"));
...
{
    // return if URI is not valid or is no BTCT: URI
...
{
    // Convert btct:// to btct:
    //
    //    Cannot handle this later, because btct:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open btct.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "BTCT.lnk";
}
...
{
    return GetAutostartDir() / "btct.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a btct.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=BTCT\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the btct app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add btct app to startup item list
