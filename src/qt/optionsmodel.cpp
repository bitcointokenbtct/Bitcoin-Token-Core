...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeBTCTAmount"))
        settings.setValue("nAnonymizeBTCTAmount", 1000);

    nAnonymizeBTCTAmount = settings.value("nAnonymizeBTCTAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeBTCTAmount"))
        SoftSetArg("-anonymizebtctamount", settings.value("nAnonymizeBTCTAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in btct.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeBTCTAmount:
            return QVariant(nAnonymizeBTCTAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeBTCTAmount:
            nAnonymizeBTCTAmount = value.toInt();
            settings.setValue("nAnonymizeBTCTAmount", nAnonymizeBTCTAmount);
            emit anonymizeBTCTAmountChanged(nAnonymizeBTCTAmount);
            break;
        case CoinControlFeatures:
